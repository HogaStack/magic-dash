...
from sqlalchemy import Boolean, DateTime, Integer, String, delete, update
...
class OtpCredentials(BaseModel):
    """SQLAlchemy版用户OTP动态口令凭据表模型"""

    __tablename__ = TABLE_NAMES["OtpCredentials"]

    user_id: Mapped[str] = mapped_column(String(255), primary_key=True)
    secret_ciphertext: Mapped[str] = mapped_column(String(255))
    is_enabled: Mapped[bool] = mapped_column(Boolean, default=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.now)
    confirmed_at: Mapped[datetime] = mapped_column(DateTime, nullable=True)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.now)
    last_used_timecode: Mapped[int] = mapped_column(Integer, nullable=True)
