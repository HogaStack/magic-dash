...
from sqlalchemy import Boolean, JSON, String, delete, func, select, update
...
class UserPermissionGroups(BaseModel):
    """SQLAlchemy版用户权限分组表模型"""

    __tablename__ = TABLE_NAMES["UserPermissionGroups"]

    permission_group_id: Mapped[str] = mapped_column(String(255), primary_key=True)
    permission_group_name: Mapped[str] = mapped_column(String(255), unique=True)
    access_rule_type: Mapped[str] = mapped_column(String(255), default="exclude")
    access_rule_keys: Mapped[object] = mapped_column(JSON, nullable=True)
