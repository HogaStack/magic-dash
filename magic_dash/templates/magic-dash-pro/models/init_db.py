...
if __name__ == "__main__":
    ...
    confirm_departments = input(
        "是否重置 \033[93mdepartments\033[0m 表？(输入 \033[93myes\033[0m 确认，其他内容跳过): "
    )
    if confirm_departments == "yes":
        Departments.truncate_departments(execute=True)
        print("\033[93mdepartments\033[0m 表已重置")
    else:
        print("已跳过 \033[93mdepartments\033[0m 表重置")

    print("\n")

    # 2. 询问是否重置用户表
    confirm_users = input(
        "是否重置 \033[93musers\033[0m 表？(输入 \033[93myes\033[0m 确认，其他内容跳过): "
    )
    if confirm_users == "yes":
        ...
        print("已跳过 \033[93musers\033[0m 表重置")
