...
class UserPermissionGroups(BaseModel):
    ...
    class Meta:
        ...
        database = db
        table_name = TABLE_NAMES["UserPermissionGroups"]
    ...
    def ensure_table(cls):
        ...
        with db.connection_context():
            ...
            cls.delete().where(
                cls.permission_group_id == AuthConfig.admin_role
            ).execute()
            ...
            with db.atomic():
                if cls.get_or_none(cls.permission_group_id == permission_group_id):
                    cls.update(
                        ...
                        is_builtin=is_builtin,
                        other_info=other_info,
                    ).where(cls.permission_group_id == permission_group_id).execute()
                else:
                    cls.create(
                        permission_group_id=permission_group_id,
                        permission_group_name=permission_group_name,
